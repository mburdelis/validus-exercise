...
class TradeDetails(BaseModel):
    ...
    def validate_underlying_and_dates(self) -> "TradeDetails":
        ...
        # Dates must follow: trade_date (deal agreed) ≤ value_date (money exchanged) ≤ delivery_date (currency transferred)
...
class HistoryEntry(BaseModel):
    ...
    No field-level validation is needed here because HistoryEntry is only ever
    constructed internally by trade.py — all values are already guaranteed valid
    by the time they reach this model (enums from _TRANSITIONS, a pre-validated
    TradeDetails snapshot, and timestamps generated by our own code).
    General rule: validate at the edges of the system where untrusted external
    data enters (TradeDetails is constructed by callers), not internally where
    all inputs are controlled by our own code.
    """
